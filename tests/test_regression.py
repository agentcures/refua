...
if os.environ.get("REFUA_RUN_REGRESSION") != "1":
    pytest.skip(
        "Regression tests are opt-in; set REFUA_RUN_REGRESSION=1 to enable.",
        allow_module_level=True,
    )

from lightning_fabric import seed_everything

from refua.boltz.main import MODEL_URL
from refua.boltz.model.model import Boltz1
...
class RegressionTester(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if "cyclic_period" not in regression_feats["feats"]:
            regression_feats["feats"]["cyclic_period"] = torch.zeros_like(
                regression_feats["feats"]["residue_index"]
            )
