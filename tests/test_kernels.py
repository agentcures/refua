...
if os.environ.get("REFUA_RUN_KERNEL_BENCH") != "1":
    pytest.skip(
        "Kernel benchmarks are opt-in; set REFUA_RUN_KERNEL_BENCH=1 to enable.",
        allow_module_level=True,
    )

triton = pytest.importorskip("triton")
if not torch.cuda.is_available():
    pytest.skip("CUDA is required for kernel benchmarks.", allow_module_level=True)

import pandas as pd
from profiling import clear_memory, current_memory, memory_measure

from refua.boltz.model.layers.pairformer import PairformerLayer
