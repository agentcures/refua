...
class ParsedResidue:
    ...
    is_standard: bool
    is_present: bool
    rdkit_bounds_constraints: list[ParsedRDKitBoundsConstraint] | None = None
    chiral_atom_constraints: list[ParsedChiralAtomConstraint] | None = None
    stereo_bond_constraints: list[ParsedStereoBondConstraint] | None = None
    planar_bond_constraints: list[ParsedPlanarBondConstraint] | None = None
    planar_ring_5_constraints: list[ParsedPlanarRing5Constraint] | None = None
    planar_ring_6_constraints: list[ParsedPlanarRing6Constraint] | None = None
...
class ParsedChain:
    ...
    affinity: bool | None = False
    affinity_mw: float | None = None
...
def get_mol(ccd: str, mols: dict, moldir: Path | None) -> Mol:
    ...
    if mol is None:
        if moldir is None:
            msg = (
                f"CCD component {ccd} is missing. Provide it in-memory or set mol_dir "
                "to allow loading from disk."
            )
            raise ValueError(msg)
