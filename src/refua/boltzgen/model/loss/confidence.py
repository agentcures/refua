...
from boltzgen.model.layers.confidence_utils import compute_frame_pred, tm_function
...
def plddt_reg_loss(
    ...
    errors = loss_fn(pred_lddt, target_lddt)
...
def pae_reg_loss(
    ...
    target_pae = torch.clamp(target_pae, max=max_dist)

    mask_pad = feats["token_pad_mask"].repeat_interleave(multiplicity, 0)
    N_res = mask_pad.sum(dim=-1).unsqueeze(1).unsqueeze(2)

    pred_ptm = tm_function(pred_pae, N_res)
    target_ptm = tm_function(target_pae, N_res)

    loss_fn = nn.HuberLoss(delta=0.03)
    errors = loss_fn(pred_ptm, target_ptm)
...
def pde_reg_loss(
    ...
    target_pde = torch.clamp(target_pde, max=max_dist)

    loss_fn = nn.HuberLoss(delta=0.03)
    errors = loss_fn(pred_pde / max_dist, target_pde / max_dist)
