...
The actual code that is executed in each pipeline step is found in `main.py`, which is a wrapper for running the .run() function of our `Task` class.
...
The possible tasks (and code files you want to inspect to understand what they are running):
    - Predict refua.boltzgen.task.predict.predict (GPU: Running BoltzGen diffusion, inverse folding, refolding, designfolding, or affinity prediction)
    - Analyze refua.boltzgen.task.analyze.analyze (CPU: Compute CPU metrics and aggregate metrics from GPU steps)
    - Filter refua.boltzgen.task.filter.filter (CPU: Fast (20s) computes ranking and writes final output files)
"""
...
### Paths and constants ####
# Get the boltzgen package root (where resources/ is located).
path_to_script = Path(__file__)
project_root = (
    path_to_script.parent.parent
)  # Go up from refua/boltzgen/cli to the package root
...
def merge_command(args: argparse.Namespace) -> None:
    ...
    def _merge_design_dir(
        ...
        metrics_frames: list[Any] = []
        seq_frames: list[Any] = []
        per_target_frames: list[Any] = []
        merged_count = 0
