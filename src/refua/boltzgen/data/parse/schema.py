...
class ParsedChain:
    ...
    sampleidx_to_specidx: np.ndarray | None = None
...
def parse_redesign_yaml(
    ...
    with path.open("r") as file:
        if path.suffix == ".yaml":
            ...
            raise ValueError(f"Unsupported file type: {path!s}")
        ...
        while True:
            ...
            for item in items_list:
                ...
                if entity_type != "file":
                    ...
                    if fuse_info["fuse"]:
                        if fuse_info["target_id"] in total_renaming:
        ...
        if isinstance(include, list):
            for list_element in include:
                ...
                if "smiles" in chain:
                    ...
                    if not success:
                        msg = "Failed to compute 3D conformer for given smiles string"
                        raise ValueError(msg)
        ...
        for chain in structure.chains:
            ...
            if chain_include_mask.sum() == 0:
                ...
                if (~included_missing_mask).sum() > 0:
                    if included_missing_mask.sum() == 0:
                        msg = f"\nThere are no resolved residues for chain {chain['name']} in {path!s}. We are removing the chain."
                    else:
