...
class DataConfig:
    ...
    extra_mol_dir: str | None = None
    disulfide_prob: float = 1.0
    disulfide_on: bool = False
    design_mask_override: str | None = None
    multiplicity: int = 1
...
class FromGeneratedDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        reference_metadata_dir: Path | None = None,
    ...
    def get_sample(self, design_dir: Path, sample_id: str | None = None) -> dict:
    ...
    def get_feat(self, path, design_mask, ss_type=None, binding_type=None):
        ...
        try:
            ...
            print(f"Failed to parse {path} with error {e}. Skipping.")
        ...
        try:
            ...
            print(f"Tokenizer failed on {path} with error {e}. Skipping.")
            raise DataFetchException from e
        ...
        try:
            ...
            print(f"Featurizer failed on {path} with error {e}. Skipping.")
            raise DataFetchException from e
...
class FromGeneratedDataModule(pl.LightningDataModule):
    def __init__(
        ...
        extra_features: list[str] | None = None,
        design_mask_override: str | None = None,
        subset_target_ids: str | None = None,
        skip_specific_ids: list[str] | None = None,
        use_new_design_mask: bool = False,
    ...
    def init_dataset(
        ...
        if self.subset_target_ids is not None:
            subset_ids = [
                l.strip() for l in open(self.subset_target_ids)
            ]
