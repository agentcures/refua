...
class PredictionDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        for path in self.yaml_paths:
            ...
            if re.search(r"_\d+\.yaml$", filename):
                raise ValueError(
                    f"Illegal YAML filename for '{path!s}': names must not end with the pattern _\\d+\\.yaml so, e.g., the ends '_001.yaml' or '_4.yaml' are not allowed."
                    "This pattern is reserved for internal file indexing. Sorry :)"
                )
            if "_native" in filename:
                raise ValueError(
                    f"Illegal YAML filename for '{path!s}': names must not contain '_native' because this substring is reserved for native structure companions. Sorry :)"
                )
        self.extra_features = (
    ...
    def get_sample(self, path: Path, sample_id: str | None = None) -> dict:
