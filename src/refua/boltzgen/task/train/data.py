...
from boltzgen.utils.resources import read_text_from_path_or_resource
...
class DatasetConfig:
    ...
    Split paths may point to local files or packaged `resources/...` paths.
    """
    ...
    filters: list[DynamicFilter] | None = None
    split: str | None = None
    symmetry_correction: bool = True
    val_group: str | None = "RCSB"
    use_train_subset: float | None = None
    moldir: str | None = None
    override_bfactor: bool | None = False
...
class TrainingDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        num_bins: int = 64,
        overfit: int | None = None,
        return_symmetries: bool | None = False,
    ...
    def __getitem__(self, idx: int) -> dict[str, Tensor]:
        ...
        try:
            ...
            print(f"Selector failed on {record.id} with error {e}. Skipping.")
            traceback.print_exc()
            return self.__getitem__(random.integers(0, len(self)))
...
class ValidationDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Structure:
        ...
        try:
            ...
            print(f"Selector failed on {sample.record_id} with error {e}. Skipping.")
            traceback.print_exc()
            return self.__getitem__(0)
        structure = tokenized.structure
...
class TrainingDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        for data_config in cfg.datasets:
            ...
            if data_config.split is not None:
                split_text = read_text_from_path_or_resource(data_config.split)
                split = {x.lower() for x in split_text.splitlines()}
        ...
        if self.monomer_split is not None:
            split_text = read_text_from_path_or_resource(self.monomer_split)
            monomer_ids = [x.lower() for x in split_text.splitlines()]
        ...
        if self.ligand_split is not None:
            split_text = read_text_from_path_or_resource(self.ligand_split)
            ligand_ids = [x.lower() for x in split_text.splitlines()]
    ...
    def setup(self, stage: str | None = None) -> None:  # noqa: ARG002 (unused)
