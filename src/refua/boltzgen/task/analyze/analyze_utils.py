...
from boltzgen.model.loss.validation import compute_subset_rmsd, factored_lddt_loss
...
def vendi_scores(
    ...
    for prefix in prefixes:
        ...
        if compute_iptms:
            mask_80 = np.array([m["design_iptm>80"] for m in all_metrics])
            mask_70 = np.array([m["design_iptm>70"] for m in all_metrics])
            mask_60 = np.array([m["design_iptm>60"] for m in all_metrics])
            mask_50 = np.array([m["design_iptm>50"] for m in all_metrics])
            scores.update(
                {
        ...
        if compute_min_int_paes:
            mask_15 = np.array([m["min_interaction_pae<1.5"] for m in all_metrics])
            mask_2 = np.array([m["min_interaction_pae<2"] for m in all_metrics])
            mask_25 = np.array([m["min_interaction_pae<2.5"] for m in all_metrics])
            mask_3 = np.array([m["min_interaction_pae<3"] for m in all_metrics])
            mask_4 = np.array([m["min_interaction_pae<4"] for m in all_metrics])
            mask_5 = np.array([m["min_interaction_pae<5"] for m in all_metrics])
            scores.update(
                {
